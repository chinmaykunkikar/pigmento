...
| Overview home | `app/page.tsx` → `components/overview/OverviewHome.tsx` | `/` — one component across ghosted first-run → indexing → live. `IdentityBand` (palette hero via `PaletteHero` + count-up coverage verdict) over `KindRows`; reads `/api/identity` (`getDesignIdentity`). Never-indexed `/` shows the ghosted identity + `FirstRunEntries` (path / folder-pick / CLI); on index it morphs in place (A7): `PaletteHero` slots resolve dashed→color (staggered), promise→verdict, `MorphSection` animates `grid-template-rows` 0fr↔1fr to collapse the entry block and expand the peer rows. Count-up signature fires on index completion via `store.signaturePending`. The home owns its own empty/indexing states; `Shell` only takes over with `EmptyState`/`IndexingCenter` off `/` |
